import type EasyModal from '.'

import { type EasyModalHOC, type EasyModalItem, type Id } from './type'
...
export function findModal<P, V>(Modal: EasyModalHOC<P, V> | Id) {
  ...
  const find = Object.values(MODAL_REGISTRY).find(
    item => item.Component === (Modal as EasyModalHOC<P, V>),
  )

  return find ? find : void 0
}

export function getEasyHoc(
  Modal: EasyModalHOC<any, any> | Id,
  where: keyof typeof EasyModal,
) {
  let warnText = ''
